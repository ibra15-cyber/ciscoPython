...
class Queue: #creating another class
    ...
    def put(self, elem): #put takes eliment and insert it in our queue list
        self.queue.insert(-1, elem) #insertion done from the front (our queue item is entering from the back aand removing from front

    def get(self):      
        if len(self.queue) > 0: #if something is in list aka len >0
            elem = self.queue[0] #get me that last one
            del self.queue[0]     #remove it 
            return elem
        else:
